...
TEST_F(ChatTemplateUtilsTest, SelectReasoningTagsForArchitectureQwen3Family) {
  for (std::string_view arch : {"qwen3", "qwen3moe"}) {
...
TEST_F(ChatTemplateUtilsTest, SelectReasoningTagsForArchitectureRejectsOthers) {
  ...
  // `qwen35` / `qwen35moe` are explicitly excluded: Qwen3.5 is a hybrid
  // SSM + attention model with multi-dimensional M-RoPE; in-place KV
  // compaction silently corrupts subsequent inferences on that family.
  EXPECT_FALSE(
      selectReasoningTagsForArchitecture(std::string("qwen35")).has_value());
  EXPECT_FALSE(selectReasoningTagsForArchitecture(std::string("qwen35moe"))
                   .has_value());
  EXPECT_FALSE(
