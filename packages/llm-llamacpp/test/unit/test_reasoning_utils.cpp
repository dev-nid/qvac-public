...
// Regression: a second reasoning block must be re-entered even when the
// previous block's close marker is still in the rolling buffer. The naive
// independent find(open) / find(close) version of the detector flipped
// inside_reasoning to true on the new open then immediately back to false
// on the stale close, masking the false→true edge that drives the second
// block's span capture in TextLlmContext.
TEST_F(ReasoningUtilsTest, UpdateBufferReentersForSecondBlockWithStaleClose) {
  ReasoningState state = makeQwen3State();

  updateReasoningBuffer("<think>first</think>", state);
  EXPECT_FALSE(state.inside_reasoning) << "first block should close";

  // Some inter-block text; the previous </think> is still inside the
  // 50-char rolling window at this point.
  updateReasoningBuffer(" inter ", state);
  EXPECT_FALSE(state.inside_reasoning) << "no markers in the new token";

  // Second open arrives. The buffer still contains the first </think>,
  // but rfind-based comparison must pick the LATER (newer) marker.
  updateReasoningBuffer("<think>", state);
  EXPECT_TRUE(state.inside_reasoning)
      << "second block must re-enter even with stale </think> in the buffer";

  updateReasoningBuffer("second</think>", state);
  EXPECT_FALSE(state.inside_reasoning) << "second block should close";
}

// Single-token piece that contains both a stale close (from the previous
// block) and a new open: the new open is the LATER marker, so the detector
// must report inside_reasoning=true.
TEST_F(ReasoningUtilsTest, UpdateBufferLatestMarkerWinsWhenBothPresent) {
  ReasoningState state = makeQwen3State();
  state.recent_output_buffer = "</think> something ";

  updateReasoningBuffer("<think>", state);
  EXPECT_TRUE(state.inside_reasoning)
      << "newer <think> at a higher index must win over older </think>";
}

// Symmetric: stale open from an earlier block + new close in the same
// update — the close is later, so state must end at false.
TEST_F(ReasoningUtilsTest, UpdateBufferStaleOpenLosesToNewerClose) {
  ReasoningState state = makeQwen3State();
  state.recent_output_buffer = "<think>middle ";
  state.inside_reasoning = true;
